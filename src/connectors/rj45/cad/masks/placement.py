...
class RJ45PlacementMaskCAD(ManifoldObject):
    ...
    def main_block(self) -> manifold3d.Manifold:
        size = [
            self.model.rj45_model.screw_tabs[0] + self.thickness * 4,
            self.model.rj45_model.body[1] + self.thickness * 8,
        ...
        return manifold3d.Manifold.cube(size, center=True).translate(
            [
                0,
                0,
                self.thickness / 2 - self.body_model.highest / 2,
            ]
        )

    def assemble(self) -> manifold3d.Manifold:
        return self.main_block.rotate([0, 0, -90]).translate(
